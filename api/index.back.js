...
async function getDirectoryEntries(requestPath, maxComputeDepth) {
    ...
    const entryDtos = await Promise.all(entries.map(async (entry) => {
        ...
        let hasPartialSize = size == null;
        if (isDirectory && !entry.isSymbolicLink && maxComputeDepth > 0) {
            ({size, hasPartialSize} = await computeSize(entryPath, 1, maxComputeDepth));
        }
 ...
 */
async function computeSize(directoryPath, currentDepth, maxDepth) {
    if (currentDepth > maxDepth) {
        return { size: 0, hasPartialSize: true };
    }
    
    const entries = await readdir(directoryPath, { withFileTypes: true });

    let directorySize = 0;
    let directoryPartialSize = false;
    await Promise.all(entries.map((entry) => {
        ...
        directorySize += size;
        directoryPartialSize |= size == null;

        const {size, partialSize } = entry.isDirectory() && !entry.isSymbolicLink() 
            ? computeSize(entryPath, currentDepth + 1, maxDepth) 
            : { size: size || 0, partialSize: size != null  };

        directorySize += size;
        directoryPartialSize |= partialSize;
    }));

    return { size: directorySize, partialSize: directoryPartialSize };
}